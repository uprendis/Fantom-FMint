...
contract FantomMintDebt is ReentrancyGuard, FantomMintErrorCodes
{
    ...
    // getMaxToMint (abstract) calculates the maximum amount of given token debt
    // which will satisfy the minimal collateral to debt ratio.
    function getMaxToMint(address _account, address _token, uint256 ratio) public view returns (uint256);
    ...
    function mustMintMax(address _token, uint256 ratio) public nonReentrant {
        // try to withdraw max amount of tokens allowed
        uint256 result = _mintMax(_token, ratio);
    ...
    function mintMax(address _token, uint256 ratio) public nonReentrant returns (uint256) {
        return _mintMax(_token, ratio);
    }
    ...
    function _mintMax(address _token, uint256 ratio) internal returns (uint256) {
        return _mint(_token, getMaxToMint(msg.sender, _token, ratio));
    }
