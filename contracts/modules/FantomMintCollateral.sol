...
contract FantomMintCollateral is ReentrancyGuard, FantomMintErrorCodes
{
    ...
    function getMaxToWithdraw(address _account, address _token, uint256 ratio) public view returns (uint256);
    ...
    // if it fails.
    function mustWithdrawMax(address _token, uint256 ratio) public nonReentrant {
        // try to withdraw max amount of tokens allowed
        uint256 result = _withdrawMax(_token, ratio);
    ...
    function withdrawMax(address _token, uint256 ratio) public nonReentrant returns (uint256) {
        return _withdrawMax(_token, ratio);
    }
    ...
    function _withdrawMax(address _token, uint256 ratio) internal returns (uint256) {
        return _withdraw(_token, getMaxToWithdraw(msg.sender, _token, ratio));
    }
}
